/*
 * multi-plane-runtime-manager.c — WRP diagnostic service for RDK CPE devices.
 *
 ...
 *
 * Usage: multi-plane-runtime-manager [catalog-dir]
 *   catalog-dir   directory containing per-plane catalog files
 *                 (default: /etc/multi-plane-runtime-manager). Each of
 *                 multi-plane-runtime-manager-triage-catalog.json, multi-plane-runtime-manager-management-catalog.json,
 *                 multi-plane-runtime-manager-control-catalog.json, multi-plane-runtime-manager-config-apply-catalog.json
 ...
 *   multi-plane-runtime-manager binds two independent socket pairs:
 *     - the original public pair (CLIENT_URL/PARODUS_URL, via Parodus)
 *     - a local-only pair (MPRM_LOCAL_RECV_URL/MPRM_LOCAL_SEND_URL, for
 ...
 *   type-9 registration so the local pair becomes multi-plane-runtime-manager's only
 *   reachable address -- was flipped on 2026-08-16 after D.1/D.3
 *   passed, then reverted the same day by direct instruction:
 *   REGISTER_WITH_PARODUS is back to 1, so multi-plane-runtime-manager registers with
 ...
 * direct instruction -- see the code comments at mprm_acl_check() and
 * mprm_notify_capability_sync() for the full design/rationale):
 *   Every EXEC request now passes through mprm_acl_check() immediately
 ...
 *   successful PUSH promote now also fires mprm_notify_capability_sync(),
...
#ifndef MULTI_PLANE_RUNTIME_MANAGER_ENABLE_DYNAMIC_PLUGINS
#define MULTI_PLANE_RUNTIME_MANAGER_ENABLE_DYNAMIC_PLUGINS 0
#endif

#ifndef MULTI_PLANE_RUNTIME_MANAGER_ENABLE_ACL
#define MULTI_PLANE_RUNTIME_MANAGER_ENABLE_ACL 0
#endif

/* ── Constants ────────────────────────────────────────────────────────────── */

#define LOG_IDENT            "multi-plane-runtime-manager"
 ...
 * own bind/connect split (multi-plane-runtime-manager binds one side to receive,
 ...
 * reference-impl/diag_legacy_framing.c's illustrative local-endpoint constant
 ...
 * doesn't silently diverge from what multi-plane-runtime-manager actually binds. */
#define MPRM_LOCAL_RECV_URL  "ipc:///run/dispatcher/diagnostics-in.sock"
#define MPRM_LOCAL_SEND_URL  "ipc:///run/dispatcher/diagnostics-out.sock"
 ...
 * the intent was for Dispatch Core to take over multi-plane-runtime-manager's public
 ...
 * before it ever reaches multi-plane-runtime-manager, forwarding allowed requests over
 ...
 * multi-plane-runtime-manager's only reachable address.
 *
 * Reverted to 1 on 2026-08-16, by direct instruction: registration is
 * needed at multi-plane-runtime-manager startup again. The public PULL/PUSH socket pair
 ...
 * A's whole point was that requests reach multi-plane-runtime-manager only *after*
 * Dispatch Core's ACL check. With registration back on, multi-plane-runtime-manager is
 * once again directly reachable from Parodus/the cloud on the public
 * pair, and mprm_acl_check() (added 2026-08-15, §13.4) is still gating
 ...
 * multi-plane-runtime-manager had before any of this project's ACL work started. If
 * that's not acceptable, either keep this at 0 until
 * acl_policy_store_query() has a real implementation, or restrict which
 * callers can reach PARODUS_URL by some means outside multi-plane-runtime-manager
 * itself.
 *
 * Added 2026-08-16: guarded with #ifndef so CMakeLists.txt's
 * MULTI_PLANE_RUNTIME_MANAGER_REGISTER_WITH_PARODUS option (default ON, matching this
...
#define CATALOG_DIR          "/etc/multi-plane-runtime-manager"
#define CATALOG_FILE_TRIAGE_DEFAULT       "multi-plane-runtime-manager-triage-catalog.json"
#define CATALOG_FILE_MANAGEMENT_DEFAULT   "multi-plane-runtime-manager-management-catalog.json"
...
#define MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_DEFAULT "/tmp/logs/multi-plane-runtime-manager.log"
...
typedef struct {
    ...
    char cfg_source[PATH_MAX];
    FILE *file_fp;
} mprm_log_cfg_t;

static mprm_log_cfg_t g_log_cfg;
static pthread_mutex_t g_log_lock = PTHREAD_MUTEX_INITIALIZER;

static char *mprm_trim_ws(char *s)
{
    char *e;
...
static int mprm_parse_bool(const char *v, int defv)
{
    if (!v) return defv;
    if (strcmp(v, "1") == 0 || strcasecmp(v, "true") == 0 || strcasecmp(v, "on") == 0 ||
        strcasecmp(v, "yes") == 0 || strcasecmp(v, "enable") == 0 || strcasecmp(v, "enabled") == 0)
        return 1;
    if (strcmp(v, "0") == 0 || strcasecmp(v, "false") == 0 || strcasecmp(v, "off") == 0 ||
        strcasecmp(v, "no") == 0 || strcasecmp(v, "disable") == 0 || strcasecmp(v, "disabled") == 0)
        return 0;
    return defv;
}

static void mprm_log_cfg_defaults(mprm_log_cfg_t *cfg)
{
    ...
    cfg->trace_api = 1;
    cfg->trace_data_io = 1;
    snprintf(cfg->file_path, sizeof(cfg->file_path), "%s", MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_DEFAULT);
    snprintf(cfg->cfg_source, sizeof(cfg->cfg_source), "%s", "(defaults-only)");
}

static void mprm_log_cfg_apply_kv(mprm_log_cfg_t *cfg, const char *key, const char *val)
{
    if (!cfg || !key || !val) return;
    if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_LOG_ENABLE") == 0)
        cfg->log_enable = mprm_parse_bool(val, cfg->log_enable);
    else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_ENABLE") == 0)
        cfg->file_enable = mprm_parse_bool(val, cfg->file_enable);
    else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_PATH") == 0)
        snprintf(cfg->file_path, sizeof(cfg->file_path), "%s", val);
    else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_LOG_TRACE_API") == 0)
        cfg->trace_api = mprm_parse_bool(val, cfg->trace_api);
    else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_LOG_TRACE_DATA_IO") == 0)
        cfg->trace_data_io = mprm_parse_bool(val, cfg->trace_data_io);
}

static void mprm_log_cfg_load_locked(void)
{
    const char *env_path;
    ...
    mprm_log_cfg_defaults(&g_log_cfg);

    env_path = getenv("MULTI_PLANE_RUNTIME_MANAGER_CONFIG_FILE");
    candidates[0] = (env_path && *env_path) ? env_path : NULL;
    candidates[1] = "/etc/multi-plane-runtime-manager/multi-plane-runtime-manager.conf";
    candidates[2] = "./multi-plane-runtime-manager.conf";

    for (i = 0; i < 3; i++) {
        if (!candidates[i]) continue;
        f = fopen(candidates[i], "r");
        if (f) {
            snprintf(g_log_cfg.cfg_source, sizeof(g_log_cfg.cfg_source), "%s", candidates[i]);
            break;
        }
    }
    ...
    if (f) {
        while (fgets(line, sizeof(line), f)) {
            ...
            *eq = '\0';
            key = mprm_trim_ws(p);
            val = mprm_trim_ws(eq + 1);
            mprm_log_cfg_apply_kv(&g_log_cfg, key, val);
        }
    ...
    {
        const char *v;
        v = getenv("MULTI_PLANE_RUNTIME_MANAGER_LOG_ENABLE");
        if (v && *v) g_log_cfg.log_enable = mprm_parse_bool(v, g_log_cfg.log_enable);

        v = getenv("MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_ENABLE");
        if (v && *v) g_log_cfg.file_enable = mprm_parse_bool(v, g_log_cfg.file_enable);

        v = getenv("MULTI_PLANE_RUNTIME_MANAGER_LOG_FILE_PATH");
        if (v && *v) snprintf(g_log_cfg.file_path, sizeof(g_log_cfg.file_path), "%s", v);

        v = getenv("MULTI_PLANE_RUNTIME_MANAGER_LOG_TRACE_API");
        if (v && *v) g_log_cfg.trace_api = mprm_parse_bool(v, g_log_cfg.trace_api);

        v = getenv("MULTI_PLANE_RUNTIME_MANAGER_LOG_TRACE_DATA_IO");
        if (v && *v) g_log_cfg.trace_data_io = mprm_parse_bool(v, g_log_cfg.trace_data_io);
    }

    g_log_cfg.loaded = 1;
}

static void mprm_log_mkdirs_for_file(const char *file_path)
{
    char tmp[PATH_MAX];
    char *p;
...
static const char *mprm_log_prio_name(int priority)
{
    switch (priority) {
...
static void mprm_log_open_file_locked(void)
{
    if (g_log_cfg.file_fp || !g_log_cfg.file_enable)
        return;

    if (!g_log_cfg.file_path[0]) {
        g_log_cfg.file_enable = 0;
        return;
    }

    mprm_log_mkdirs_for_file(g_log_cfg.file_path);
    g_log_cfg.file_fp = fopen(g_log_cfg.file_path, "a");
    if (!g_log_cfg.file_fp) {
        g_log_cfg.file_enable = 0;
        return;
    }

    setvbuf(g_log_cfg.file_fp, NULL, _IOLBF, 0);
}

static void mprm_log_emit_locked(int priority, const char *fmt, va_list ap)
{
    va_list ap_sys;

    if (!g_log_cfg.log_enable)
        return;

    va_copy(ap_sys, ap);
    vsyslog(priority, fmt, ap_sys);
    va_end(ap_sys);

    if (g_log_cfg.file_enable) {
        va_list ap_file;
        char msg[2048];
        char ts[64];
        time_t now;
        ...
        mprm_log_open_file_locked();
        if (!g_log_cfg.file_fp)
            return;

        va_copy(ap_file, ap);
        vsnprintf(msg, sizeof(msg), fmt, ap_file);
        va_end(ap_file);
        ...
        fprintf(g_log_cfg.file_fp, "%s [%s] %s\n", ts, mprm_log_prio_name(priority), msg);
    }
}

static void mprm_log_message(int priority, const char *fmt, ...)
{
    va_list ap;
    pthread_mutex_lock(&g_log_lock);
    if (!g_log_cfg.loaded)
        mprm_log_cfg_load_locked();
    va_start(ap, fmt);
    mprm_log_emit_locked(priority, fmt, ap);
    va_end(ap);
    pthread_mutex_unlock(&g_log_lock);
}

static int mprm_log_trace_api_enabled(void)
{
    int enabled;
    pthread_mutex_lock(&g_log_lock);
    if (!g_log_cfg.loaded)
        mprm_log_cfg_load_locked();
    enabled = (g_log_cfg.log_enable && g_log_cfg.trace_api);
    pthread_mutex_unlock(&g_log_lock);
    return enabled;
}

static int mprm_log_trace_data_enabled(void)
{
    int enabled;
    pthread_mutex_lock(&g_log_lock);
    if (!g_log_cfg.loaded)
        mprm_log_cfg_load_locked();
    enabled = (g_log_cfg.log_enable && g_log_cfg.trace_data_io);
    pthread_mutex_unlock(&g_log_lock);
    return enabled;
}

static void mprm_log_shutdown(void)
{
    pthread_mutex_lock(&g_log_lock);
    if (g_log_cfg.file_fp) {
        fclose(g_log_cfg.file_fp);
        g_log_cfg.file_fp = NULL;
    }
    pthread_mutex_unlock(&g_log_lock);
}

#define syslog mprm_log_message

#define MPRM_API_LOG(fmt, ...) \
    do { if (mprm_log_trace_api_enabled()) syslog(LOG_DEBUG, "api: " fmt, ##__VA_ARGS__); } while (0)

#define MPRM_DATA_LOG(fmt, ...) \
    do { if (mprm_log_trace_data_enabled()) syslog(LOG_INFO, "data: " fmt, ##__VA_ARGS__); } while (0)
...
static unsigned long g_meta_type_errors = 0;
static unsigned long g_meta_conflicts = 0;
static unsigned long g_meta_policy_rejects = 0;
...
typedef struct {
    ...
    const char     *filename;           /* e.g. "multi-plane-runtime-manager-triage-catalog.json" */
...
static plane_catalog_t g_planes[PLANE_COUNT] = {
    { "triage",       CATALOG_FILE_TRIAGE_DEFAULT,       NULL, 0, {0}, PTHREAD_MUTEX_INITIALIZER },
    { "management",   CATALOG_FILE_MANAGEMENT_DEFAULT,   NULL, 0, {0}, PTHREAD_MUTEX_INITIALIZER },
    { "control",      CATALOG_FILE_CONTROL_DEFAULT,      NULL, 0, {0}, PTHREAD_MUTEX_INITIALIZER },
    { "config-apply", CATALOG_FILE_CONFIG_APPLY_DEFAULT, NULL, 0, {0}, PTHREAD_MUTEX_INITIALIZER },
};
...
typedef struct {
    ...
    char file_triage[PATH_MAX];
    char file_management[PATH_MAX];
    char file_control[PATH_MAX];
    char file_config_apply[PATH_MAX];
...
static void catalog_cfg_defaults(void)
{
    memset(&g_catalog_cfg, 0, sizeof(g_catalog_cfg));
    catalog_cfg_set(g_catalog_cfg.catalog_dir, sizeof(g_catalog_cfg.catalog_dir), CATALOG_DIR);
    catalog_cfg_set(g_catalog_cfg.file_triage, sizeof(g_catalog_cfg.file_triage), CATALOG_FILE_TRIAGE_DEFAULT);
    catalog_cfg_set(g_catalog_cfg.file_management, sizeof(g_catalog_cfg.file_management), CATALOG_FILE_MANAGEMENT_DEFAULT);
    catalog_cfg_set(g_catalog_cfg.file_control, sizeof(g_catalog_cfg.file_control), CATALOG_FILE_CONTROL_DEFAULT);
    catalog_cfg_set(g_catalog_cfg.file_config_apply, sizeof(g_catalog_cfg.file_config_apply), CATALOG_FILE_CONFIG_APPLY_DEFAULT);
    catalog_cfg_set(g_catalog_cfg.used_path, sizeof(g_catalog_cfg.used_path), "(defaults-only)");
}

static void catalog_cfg_load_file(void)
{
    ...
    if (g_catalog_cfg.loaded)
        return;

    catalog_cfg_defaults();
    ...
    for (i = 0; i < 3; i++) {
        ...
        f = fopen(candidates[i], "r");
        if (f) {
            catalog_cfg_set(g_catalog_cfg.used_path, sizeof(g_catalog_cfg.used_path), candidates[i]);
            break;
        }
    }

    if (!f) {
        g_catalog_cfg.loaded = 1;
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_DIR") == 0)
            catalog_cfg_set(g_catalog_cfg.catalog_dir, sizeof(g_catalog_cfg.catalog_dir), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_TRIAGE") == 0)
            catalog_cfg_set(g_catalog_cfg.file_triage, sizeof(g_catalog_cfg.file_triage), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_MANAGEMENT") == 0)
            catalog_cfg_set(g_catalog_cfg.file_management, sizeof(g_catalog_cfg.file_management), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONTROL") == 0)
            catalog_cfg_set(g_catalog_cfg.file_control, sizeof(g_catalog_cfg.file_control), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONFIG_APPLY") == 0)
            catalog_cfg_set(g_catalog_cfg.file_config_apply, sizeof(g_catalog_cfg.file_config_apply), val);
    }
...
static const char *catalog_cfg_value(const char *env_key)
{
    const char *ev;
    if (!env_key)
        return NULL;

    catalog_cfg_load_file();

    ev = getenv(env_key);
    if (ev && *ev)
        return ev;

    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_DIR") == 0)
        return g_catalog_cfg.catalog_dir;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_TRIAGE") == 0)
        return g_catalog_cfg.file_triage;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_MANAGEMENT") == 0)
        return g_catalog_cfg.file_management;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONTROL") == 0)
        return g_catalog_cfg.file_control;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONFIG_APPLY") == 0)
        return g_catalog_cfg.file_config_apply;

    return NULL;
}

static const char *catalog_filename_for_plane(int plane_index)
{
    const char *name = NULL;
    switch (plane_index) {
        case 0:
            name = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_TRIAGE");
            break;
        case 1:
            name = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_MANAGEMENT");
            break;
        case 2:
            name = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONTROL");
            break;
        case 3:
            name = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_FILE_CONFIG_APPLY");
            break;
        default:
            break;
    }
    ...
    if (!name || !*name)
        return (plane_index >= 0 && plane_index < PLANE_COUNT) ? g_planes[plane_index].filename : NULL;
    return name;
}
...
{
    const char *cfg_dir = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_DIR");
    const char *dir = catalog_dir_override ? catalog_dir_override : ((cfg_dir && *cfg_dir) ? cfg_dir : CATALOG_DIR);
    int loaded = 0;

    for (int i = 0; i < PLANE_COUNT; i++) {
        const char *filename = catalog_filename_for_plane(i);
        char path[512];
        snprintf(path, sizeof(path), "%s/%s", dir, filename ? filename : g_planes[i].filename);
...
typedef struct {
    ...
    char enable[16];
    char dir[PATH_MAX];
    char dir_triage[PATH_MAX];
    char dir_management[PATH_MAX];
    ...
    char verify_mode[32];
} plugin_runtime_cfg_t;

static plugin_runtime_cfg_t g_plugin_cfg;

static const char *plugin_cfg_value(const char *env_key);
...
static void set_cfg_val(char *dst, size_t dst_sz, const char *src)
{
...
static void plugin_cfg_defaults(void)
{
    memset(&g_plugin_cfg, 0, sizeof(g_plugin_cfg));
    set_cfg_val(g_plugin_cfg.enable, sizeof(g_plugin_cfg.enable), "0");
    set_cfg_val(g_plugin_cfg.dir, sizeof(g_plugin_cfg.dir), PLUGIN_DIR_DEFAULT);
    set_cfg_val(g_plugin_cfg.dir_triage, sizeof(g_plugin_cfg.dir_triage), "");
    set_cfg_val(g_plugin_cfg.dir_management, sizeof(g_plugin_cfg.dir_management), "");
...
static void append_dir_csv(char *dst, size_t dst_sz, const char *dir)
{
    size_t cur;
    if (!dst || dst_sz == 0 || !dir || !*dir)
        return;
    ...
    if (cur > 0) {
        snprintf(dst + cur, dst_sz - cur, ",%s", dir);
    } else {
        snprintf(dst, dst_sz, "%s", dir);
    }
}

static const char *plugin_cfg_effective_dirs(void)
{
    const char *triage = plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_TRIAGE");
    const char *management = plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_MANAGEMENT");
    const char *control = plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONTROL");
    const char *config_apply = plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONFIG_APPLY");
    const char *generic = plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR");

    g_plugin_cfg.dir_effective[0] = '\0';

    append_dir_csv(g_plugin_cfg.dir_effective, sizeof(g_plugin_cfg.dir_effective), triage);
    append_dir_csv(g_plugin_cfg.dir_effective, sizeof(g_plugin_cfg.dir_effective), management);
    append_dir_csv(g_plugin_cfg.dir_effective, sizeof(g_plugin_cfg.dir_effective), control);
    append_dir_csv(g_plugin_cfg.dir_effective, sizeof(g_plugin_cfg.dir_effective), config_apply);

    if (g_plugin_cfg.dir_effective[0] == '\0') {
        append_dir_csv(g_plugin_cfg.dir_effective, sizeof(g_plugin_cfg.dir_effective),
                       (generic && *generic) ? generic : PLUGIN_DIR_DEFAULT);
    }

    return g_plugin_cfg.dir_effective;
}

static void plugin_cfg_load_file(void)
{
    const char *env_path = getenv("MULTI_PLANE_RUNTIME_MANAGER_CONFIG_FILE");
    const char *candidates[3];
    FILE *f = NULL;
    ...
    for (i = 0; i < 3; i++) {
        ...
        f = fopen(candidates[i], "r");
        if (f) {
            set_cfg_val(g_plugin_cfg.used_path, sizeof(g_plugin_cfg.used_path), candidates[i]);
            break;
        }
    }

    if (!f) {
        set_cfg_val(g_plugin_cfg.used_path, sizeof(g_plugin_cfg.used_path), "(defaults-only)");
        g_plugin_cfg.loaded = 1;
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_ENABLE") == 0)
            set_cfg_val(g_plugin_cfg.enable, sizeof(g_plugin_cfg.enable), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR") == 0)
            set_cfg_val(g_plugin_cfg.dir, sizeof(g_plugin_cfg.dir), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_TRIAGE") == 0)
            set_cfg_val(g_plugin_cfg.dir_triage, sizeof(g_plugin_cfg.dir_triage), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_MANAGEMENT") == 0)
            set_cfg_val(g_plugin_cfg.dir_management, sizeof(g_plugin_cfg.dir_management), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONTROL") == 0)
            ...
            set_cfg_val(g_plugin_cfg.conflict_policy, sizeof(g_plugin_cfg.conflict_policy), val);
        else if (strcmp(key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_VERIFY_MODE") == 0)
            set_cfg_val(g_plugin_cfg.verify_mode, sizeof(g_plugin_cfg.verify_mode), val);
    }
...
static const char *plugin_cfg_value(const char *env_key)
{
    ...
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_ENABLE") == 0)
        return g_plugin_cfg.enable;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR") == 0)
        return g_plugin_cfg.dir;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_TRIAGE") == 0)
        return g_plugin_cfg.dir_triage;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_MANAGEMENT") == 0)
        return g_plugin_cfg.dir_management;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONTROL") == 0)
    ...
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_CONFLICT_POLICY") == 0)
        return g_plugin_cfg.conflict_policy;
    if (strcmp(env_key, "MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_VERIFY_MODE") == 0)
        return g_plugin_cfg.verify_mode;
    return NULL;
}

static int env_bool(const char *key, int defv)
{
    const char *v = plugin_cfg_value(key);
    if (!v) return defv;
    if (strcmp(v, "1") == 0 || strcasecmp(v, "true") == 0 || strcasecmp(v, "on") == 0)
        return 1;
    if (strcmp(v, "0") == 0 || strcasecmp(v, "false") == 0 || strcasecmp(v, "off") == 0)
...
static void mprm_plugin_log(int level, const char *fmt, ...)
{
    va_list ap;
    char buf[1024];
    va_start(ap, fmt);
...
static const char *mprm_plugin_get_config(const char *key)
{
    if (!key) return NULL;
    if (strcmp(key, "plugin.dir") == 0)
        return plugin_cfg_effective_dirs();
    if (strcmp(key, "plugin.dir.triage") == 0)
        return plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_TRIAGE");
    if (strcmp(key, "plugin.dir.management") == 0)
        return plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_MANAGEMENT");
    if (strcmp(key, "plugin.dir.control") == 0)
        return plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONTROL");
    if (strcmp(key, "plugin.dir.config_apply") == 0)
        return plugin_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_PLUGIN_DIR_CONFIG_APPLY");
    if (strcmp(key, "plugin.verify_mode") == 0)
...
static int catalog_tool_exists_cb(const char *tool_name, void *ctx)
{
    (void)ctx;
    int exists = 0;
 ...
 * entry may now declare `"type": "dynamic"` (see multi-plane-runtime-manager-triage-catalog.json's
 ...
 * followed by a newline). Exists to preserve multi-plane-runtime-manager-triage-catalog.json's
 ...
 * multi-plane-runtime-manager-triage-catalog.json's "active_connections" entry previously
...
static void pack_bool_kv(msgpack_packer *pk, const char *key, int value)
{
...
static char *dup_bytes_to_cstr(const void *buf, size_t len)
{
    char *out = (char *)malloc(len + 1);
    if (!out) return NULL;
    if (buf && len > 0) memcpy(out, buf, len);
    out[len] = '\0';
    return out;
}
 ...
 *   { "msg_type": 9, "service_name": "multi-plane-runtime-manager", "url": CLIENT_URL }
 */
 ...
 */
static void *build_response_payload_ex(const char *tool, int exit_code,
                                       const char *output,
                                       const req_metadata_t *meta,
                                       int include_meta_echo,
                                       size_t *out_len)
{
    msgpack_sbuffer sbuf;
    msgpack_packer  pk;
    ...
    int map_fields = 3;
    if (include_meta_echo) map_fields++;
    ...
    if (include_meta_echo) {
        int meta_fields = 0;
        msgpack_pack_str(&pk, 16);
        msgpack_pack_str_body(&pk, "metadata_applied", 16);

        if (meta && meta->plane) meta_fields++;
        if (meta && meta->plane_type) meta_fields++;
        if (meta && meta->request_type) meta_fields++;
        if (meta && meta->request_sub_type) meta_fields++;
        if (meta && meta->has_static) meta_fields++;
        if (meta && meta->has_dynamic) meta_fields++;

        msgpack_pack_map(&pk, meta_fields);
        if (meta && meta->plane) pack_str_kv(&pk, "plane", meta->plane);
        if (meta && meta->plane_type) pack_str_kv(&pk, "plane_type", meta->plane_type);
        if (meta && meta->request_type) pack_str_kv(&pk, "request_type", meta->request_type);
        if (meta && meta->request_sub_type) pack_str_kv(&pk, "request_sub_type", meta->request_sub_type);
        if (meta && meta->has_static) pack_bool_kv(&pk, "static", meta->static_flag);
        if (meta && meta->has_dynamic) pack_bool_kv(&pk, "dynamic", meta->dynamic_flag);
    }
...
{
    return build_response_payload_ex(tool, exit_code, output, NULL, 0, out_len);
}
 ...
 *
 * `mprm_acl_check()` is a thin wrapper, not a new interface: it calls
 ...
 * multi-plane-runtime-manager. There is no shared header connecting multi-plane-runtime-manager's build
 ...
 * item 4), and multi-plane-runtime-manager inherits that exact same open dependency
 ...
 */
#if MULTI_PLANE_RUNTIME_MANAGER_ENABLE_ACL
typedef struct {
...
static bool mprm_acl_check(const caller_identity_t *caller, const char *tool)
{
...
static void handle_request(wrp_req_t *req)
{
    ...
    meta_cfg_t meta_cfg = meta_cfg_from_env();
    meta_status_t meta_status = META_OK;
    meta_decision_t meta_decision = META_DECISION_BASELINE;
    int meta_allow_override = 0;
    char meta_reason[128] = {0};
    int include_meta_echo = 0;
    const char *static_s = "na";
    const char *dynamic_s = "na";
    ...
    if (req->payload && req->payload_len > 0)
        decode_request_payload(req->payload, req->payload_len, &tool, &cmd, &req_plane, &req->meta);

    if (req->meta.has_static)
        static_s = req->meta.static_flag ? "true" : "false";
    if (req->meta.has_dynamic)
        dynamic_s = req->meta.dynamic_flag ? "true" : "false";
     ...
     * the meaningful "payload data" to have in the log. */
        MPRM_DATA_LOG("INPUT message: uuid=%s src=%s tool=%s command='%s' req_plane=%s plane=%s plane_type=%s static=%s dynamic=%s",
           req->transaction_uuid ? req->transaction_uuid : "?",
           req->source ? req->source : "?",
           tool,
           cmd ? cmd : "",
              req_plane ? req_plane : "(none)",
           req->meta.plane ? req->meta.plane : "(none)",
           req->meta.plane_type ? req->meta.plane_type : "(none)",
           static_s,
           dynamic_s);
    ...
    if (meta_cfg.enabled) {
        ...
        if (meta_status != META_OK) {
            int meta_exit = meta_error_exit_code(meta_status);
            const char *meta_token = meta_error_token(meta_status);
            size_t dpl = 0;
            size_t dwrp_len = 0;
            void *dpayload = NULL;
            void *dwrp = NULL;

            if (meta_status == META_ERR_TYPE) g_meta_type_errors++;
            else if (meta_status == META_ERR_CONFLICT) g_meta_conflicts++;
            else if (meta_status == META_ERR_POLICY) g_meta_policy_rejects++;

            syslog(LOG_WARNING,
                   "metadata reject: uuid=%s tool=%s token=%s reason=%s plane=%s plane_type=%s static=%s dynamic=%s",
                   req->transaction_uuid ? req->transaction_uuid : "?",
                   tool ? tool : "unknown",
                   meta_token,
                   meta_reason[0] ? meta_reason : "n/a",
                   req->meta.plane ? req->meta.plane : "(none)",
                   req->meta.plane_type ? req->meta.plane_type : "(none)",
                   static_s,
                   dynamic_s);

            dpayload = build_response_payload(tool, meta_exit, meta_token, &dpl);
            ...
            dwrp = build_wrp_response(dresp_src, dresp_dst, duuid, dpayload, dpl, &dwrp_len);
            free(dpayload);
            if (dwrp && dwrp_len > 0) {
                int drc = nn_send(req->reply_sock, dwrp, dwrp_len, 0);
                if (drc < 0)
                    syslog(LOG_ERR, "metadata-reject nn_send failed: %s (uuid=%s)",
                           nn_strerror(nn_errno()), duuid);
            }
            free(dwrp);
            free(tool);
            free(cmd);
        ...
        include_meta_echo = 0;
        {
            const char *echo_env = getenv("MULTI_PLANE_RUNTIME_MANAGER_METADATA_ECHO");
            if (echo_env && strcmp(echo_env, "1") == 0)
                include_meta_echo = 1;
        }
    } else {
        meta_allow_override = (cmd && *cmd) ? 1 : 0;
    }
     ...
     * caller_identity_t's *shape* is final; what multi-plane-runtime-manager populates
...
#if MULTI_PLANE_RUNTIME_MANAGER_ENABLE_DYNAMIC_PLUGINS
    if (g_plugin_manager) {
        diag_invoke_req_t preq;
        diag_invoke_resp_t presp;
        plugin_invoke_result_t prc;
        int p_exit = 0;
        char *p_out = NULL;

        memset(&preq, 0, sizeof(preq));
        memset(&presp, 0, sizeof(presp));
        preq.transaction_uuid = req->transaction_uuid;
        preq.source = req->source;
        preq.tool = tool;
        preq.payload = req->payload;
        preq.payload_len = req->payload_len;

        prc = plugin_manager_invoke(g_plugin_manager, tool, &preq, &presp);
        if (prc != PLUGIN_INVOKE_NOT_FOUND) {
            size_t pl = 0;
            size_t wrp_len = 0;
            void *payload = NULL;
            void *wrp = NULL;

            if (prc == PLUGIN_INVOKE_OK) {
                p_exit = presp.exit_code;
                p_out = dup_bytes_to_cstr(presp.out, presp.out_len);
                if (!p_out) p_out = strdup("");
            } else if (prc == PLUGIN_INVOKE_ERR_API_VERSION) {
                p_exit = 72;
                p_out = strdup("ERR_PLUGIN_API_VERSION");
            } else if (prc == PLUGIN_INVOKE_ERR_UNAVAILABLE) {
                p_exit = 71;
                p_out = strdup("ERR_PLUGIN_UNAVAILABLE");
            } else {
                p_exit = 70;
                p_out = strdup("ERR_PLUGIN_INVOKE");
            }

            syslog(LOG_INFO, "plugin exec tool=%s result=%d exit=%d",
                   tool, (int)prc, p_exit);

            payload = build_response_payload_ex(tool, p_exit, p_out ? p_out : "",
                                                include_meta_echo ? &req->meta : NULL,
                                                include_meta_echo, &pl);
            ...
            wrp = build_wrp_response(resp_src, resp_dst, uuid, payload, pl, &wrp_len);
            free(payload);

            if (wrp && wrp_len > 0) {
                int rc = nn_send(req->reply_sock, wrp, wrp_len, 0);
                if (rc < 0)
                    syslog(LOG_ERR, "nn_send failed (plugin path): %s (uuid=%s)",
                           nn_strerror(nn_errno()), uuid);
                else
                    syslog(LOG_INFO, "sent response uuid=%s tool=%s exit=%d bytes=%zu (plugin)",
                           uuid, tool ? tool : "?", p_exit, wrp_len);
            }
    ...
    if (!entry) {
         ...
         * entry in multi-plane-runtime-manager-triage-catalog.json), as opposed to the default
         ...
         * triage). multi-plane-runtime-manager does not branch on this; it's logged
        ...
        int is_override = (cmd && *cmd) ? 1 : 0;
        if (meta_cfg.enabled)
            is_override = is_override && meta_allow_override;
    ...
    if (cmd && *cmd) {
         ...
         * neither changes execution behavior here. */
         syslog(LOG_INFO, "exec tool=%s type=%s plane=%s cmd='%s' timeout=%ds meta_decision=%s",
               tool, is_dynamic ? "dynamic" : "static",
             plane_dup ? plane_dup : "unset", cmd, timeout_sec,
             meta_decision_name(meta_decision));
     ...
     * already covers. */
        syslog(LOG_INFO, "OUTPUT message: uuid=%s tool=%s exit=%d stdout='%s' plane=%s plane_type=%s static=%s dynamic=%s meta_decision=%s",
           req->transaction_uuid ? req->transaction_uuid : "?",
    ...
    size_t pl = 0;
    void *payload = build_response_payload_ex(tool, exit_code, output ? output : "",
                                              include_meta_echo ? &req->meta : NULL,
                                              include_meta_echo, &pl);
    ...
    if (wrp && wrp_len > 0) {
        ...
        if (rc < 0)
            ...
            MPRM_DATA_LOG("sent response uuid=%s tool=%s exit=%d bytes=%zu",
    ...
    wrp_req_free(req);
    free(req);
    MPRM_API_LOG("handle_request end");
}

/* ── Local protocol: DESCRIBE / HEALTH / PUSH / CHANGED (§15 B.3) ───────────
 *
 * Requests with an inner-payload "kind" field are routed to dedicated
 * handlers; absent/unknown kind falls back to EXEC semantics.
 */
 ...
 * ("multi-plane-runtime-manager's own build_registration() already sends an unsolicited
...
static void send_changed_notification(int sock, const char *plane, long version)
{
    ...
    /* Unsolicited: source is multi-plane-runtime-manager's own address; dest/uuid are
...
/*
 * mprm_notify_capability_sync() — Added 2026-08-15 (§13.4), "outbound
 ...
 *
 * Authentication is inherited for free: this rides multi-plane-runtime-manager's
...
static void handle_push_request(wrp_req_t *req)
{
    ...
    if (outcome.status == PUSH_OK && plane) {
        ...
        mprm_notify_capability_sync(plane, outcome.new_version);
    }
...
{
    char *kind = NULL;
    MPRM_API_LOG("handle_local_request begin uuid=%s", req && req->transaction_uuid ? req->transaction_uuid : "?");
    if (req->payload && req->payload_len > 0)
        kind = peek_message_kind(req->payload, req->payload_len);

    if (!kind || strcmp(kind, "EXEC") == 0) {
        MPRM_API_LOG("handle_local_request kind=EXEC");
        free(kind);
        handle_request(req); /* unchanged; takes ownership of req */
        return;
    }
    if (strcmp(kind, "DESCRIBE") == 0) { MPRM_API_LOG("handle_local_request kind=DESCRIBE"); free(kind); handle_describe_request(req); return; }
    if (strcmp(kind, "HEALTH") == 0)   { MPRM_API_LOG("handle_local_request kind=HEALTH"); free(kind); handle_health_request(req);   return; }
    if (strcmp(kind, "PUSH") == 0)     { MPRM_API_LOG("handle_local_request kind=PUSH"); free(kind); handle_push_request(req);     return; }
...
static void service_one_message(int recv_sock, int reply_sock, int from_local)
{
    ...
    MPRM_API_LOG("service_one_message recv_sock=%d reply_sock=%d from_local=%d", recv_sock, reply_sock, from_local);
    if (rc < 0) {
...
{
    const char *cfg_catalog_dir = catalog_cfg_value("MULTI_PLANE_RUNTIME_MANAGER_CATALOG_DIR");
    if (cfg_catalog_dir && *cfg_catalog_dir)
        catalog_dir_override = cfg_catalog_dir;
    ...
    openlog(LOG_IDENT, LOG_PID | LOG_CONS, LOG_DAEMON);
    {
        ...
        if (!g_log_cfg.loaded)
            mprm_log_cfg_load_locked();
        le = g_log_cfg.log_enable;
        lfe = g_log_cfg.file_enable;
        lta = g_log_cfg.trace_api;
        ltd = g_log_cfg.trace_data_io;
        snprintf(lfp, sizeof(lfp), "%s", g_log_cfg.file_path);
        snprintf(lsrc, sizeof(lsrc), "%s", g_log_cfg.cfg_source);
        pthread_mutex_unlock(&g_log_lock);
        syslog(LOG_INFO,
               "logging config: enable=%d file_enable=%d file_path=%s trace_api=%d trace_data_io=%d cfg_source=%s",
               le, lfe, lfp, lta, ltd, lsrc);
    }
...
#if MULTI_PLANE_RUNTIME_MANAGER_ENABLE_DYNAMIC_PLUGINS
    {
        ...
        host_api.log_fn = mprm_plugin_log;
        host_api.get_config_fn = mprm_plugin_get_config;
        ...
        } else if (plugin_manager_start(g_plugin_manager) != 0) {
            syslog(LOG_ERR, "plugin_manager_start failed; dynamic plugins disabled");
     ...
     * Dispatch Core side doesn't exist in this deployment), multi-plane-runtime-manager
    ...
    if (g_local_pull_sock < 0) {
        ...
        if (nn_bind(g_local_pull_sock, MPRM_LOCAL_RECV_URL) < 0) {
            syslog(LOG_WARNING, "local endpoint disabled: nn_bind(%s) failed: %s",
                   MPRM_LOCAL_RECV_URL, nn_strerror(nn_errno()));
    ...
    if (g_local_pull_sock >= 0) {
        ...
        if (g_local_push_sock < 0) {
            ...
            if (nn_connect(g_local_push_sock, MPRM_LOCAL_SEND_URL) < 0) {
                syslog(LOG_WARNING, "local endpoint disabled: nn_connect(%s) failed: %s",
                       MPRM_LOCAL_SEND_URL, nn_strerror(nn_errno()));
    ...
    if (g_local_enabled)
        syslog(LOG_INFO, "local endpoint active: recv=%s send=%s",
             MPRM_LOCAL_RECV_URL, MPRM_LOCAL_SEND_URL);
    else
...
shutdown:
    ...
    if (g_plugin_manager) {
        ...
        plugin_manager_get_metrics(g_plugin_manager, &pm);
        syslog(LOG_INFO,
               "plugin metrics: loaded=%llu failed=%llu tools_registered=%llu invocations=%llu invoke_errors=%llu reloads=%llu",
               (unsigned long long)pm.plugins_loaded_total,
               (unsigned long long)pm.plugins_failed_total,
               (unsigned long long)pm.tools_registered_total,
               (unsigned long long)pm.plugin_invocations_total,
               (unsigned long long)pm.plugin_invocation_errors_total,
               (unsigned long long)pm.plugin_reload_total);
        plugin_manager_destroy(g_plugin_manager);
        g_plugin_manager = NULL;
    }
#endif
