# Multi-Plane Runtime Manager Software Requirements Specification (SRS)
...
> `multi-plane-runtime-manager.c` actually links against.
>
...
This document defines the requirements for Multi-Plane Runtime Manager, a diagnostic execution service used on RDK CPE devices. It captures:
...
Multi-Plane Runtime Manager shall:
...
Multi-Plane Runtime Manager runs on the device and communicates locally with Parodus.

High-level interaction:
1. OPS Gateway sends request to Parodus.
2. Parodus forwards WRP request to Multi-Plane Runtime Manager.
3. Multi-Plane Runtime Manager executes command and sends WRP response to Parodus.
...
- nanomsg library (direct raw-socket integration with Parodus — see
  `multi-plane-runtime-manager.c`'s own header comment: "no libparodus dependency").
- msgpack-c library.
- cJSON library.
...
- Default catalog path: `/etc/multi-plane-runtime-manager/catalog.json`.
...
- Each tool entry should include:
  ...
  - `plane` as string, optional, one of `config-apply`, `management`,
    ...
    consumed by multi-plane-runtime-manager's own execution logic.
...
`MAX_OUTPUT_BYTES` while `multi-plane-runtime-manager` stopped reading could block
...
observability only — multi-plane-runtime-manager's execution behavior does not depend
on `plane`.
...
compiled `multi-plane-runtime-manager.c`; the previously-recorded "source filename
...
5. ~~A caller-supplied `command` field in the request payload
   ...
   **Related, added 2026-08-14 (FR-25b):** a catalog tool may now
   ...
   `adhoc_diagnostic` in `multi-plane-runtime-manager-triage-catalog.json`), not a bug or a
...
6. **No access-control check exists on who may invoke a tool** — added
   ...
   message to `multi-plane-runtime-manager` through Parodus can invoke any catalog
   tool. **Design added 2026-08-14, scoped to Phase 2 by direct
   instruction — see `docs/24_diag_server_merge_plan.md` §10.**
   Recommended approach: retire multi-plane-runtime-manager's direct Parodus
   ...
   ever forwarding to multi-plane-runtime-manager, and returns
   `{"tool", "exit_code": <nonzero>, "stdout": "access denied"}` on
   denial without multi-plane-runtime-manager seeing the request at all. Not yet
...
- Runtime behavior reference: `multi-plane-runtime-manager.c`
