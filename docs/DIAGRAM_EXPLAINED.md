...
Acts as the intermediary between the cloud (OPS Gateway) and the device service (Multi-Plane Runtime Manager).
Receives requests from the cloud on one socket and forwards them to Multi-Plane Runtime Manager on another.
...
in `multi-plane-runtime-manager.c`) — actual captured output ranges from 0 bytes
...
`multi-plane-runtime-manager.c`, so a hung command blocks its worker thread
...
### Message Types Used in Multi-Plane Runtime Manager

| msg_type | Name | Direction | Purpose |
|----------|------|-----------|---------|
| **3** | Request / Response | Bidirectional | Cloud sends diagnostic command; device returns result |
| **9** | Registration | Device → Cloud | Device announces itself as `multi-plane-runtime-manager` at startup |
...
### How WRP Flows Through Multi-Plane Runtime Manager

1. Cloud sends **WRP msg_type=3** to Parodus with payload `{"tool": "cpu_info", "command": ""}`
2. Parodus forwards to Multi-Plane Runtime Manager via nanomsg PULL socket
3. Multi-Plane Runtime Manager decodes the outer WRP envelope to extract routing metadata
4. Multi-Plane Runtime Manager decodes the inner payload to get tool name and command
5. Multi-Plane Runtime Manager executes the command and packages the result
6. Multi-Plane Runtime Manager builds a **WRP msg_type=3** response with payload `{"tool": "cpu_info", "exit_code": 0, "stdout": "..."}`
7. Multi-Plane Runtime Manager sends via nanomsg PUSH socket back to Parodus
...
- **Standardized routing**: Works across all RDK device services, not just Multi-Plane Runtime Manager
...
### Socket Configuration in Multi-Plane Runtime Manager
...
```c
PARODUS_URL  =  "tcp://127.0.0.1:6666"   // Parodus broker — Multi-Plane Runtime Manager connects TO this (PUSH)
CLIENT_URL   =  "tcp://127.0.0.1:6669"   // Multi-Plane Runtime Manager's own address — Parodus pushes TO this (PULL)
```

Both are **localhost-only**. Parodus handles all cloud-facing TLS/WebSocket connections independently — Multi-Plane Runtime Manager never communicates with the cloud directly.
...
At startup, Multi-Plane Runtime Manager sends a **WRP msg_type=9** to Parodus identifying itself:

```json
{ "msg_type": 9, "service_name": "multi-plane-runtime-manager", "url": "tcp://127.0.0.1:6669" }
```

This tells Parodus where to route all incoming requests addressed to `multi-plane-runtime-manager`.

### Connection Retry / Backoff

If Parodus is not ready at startup, Multi-Plane Runtime Manager retries the PUSH socket connection using **exponential backoff**:
...
┌───────────────────────── Device ──────────────────────────┐
│                                                            │
│   Multi-Plane Runtime Manager ──PUSH──► Parodus :6666                     │
│                                                            │
│   Multi-Plane Runtime Manager ◄──PULL── Parodus :6669                     │
│                                                            │
...
Multi-Plane Runtime Manager only knows two addresses (`127.0.0.1:6666` and `127.0.0.1:6669`). All cloud endpoint configuration (hostnames, ports, TLS certificates) lives in Parodus — completely decoupled from Multi-Plane Runtime Manager.
